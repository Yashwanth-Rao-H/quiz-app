...
def difficult():
    ...
    global h
    global difficult_q, difficult_a
...
def showMark(mark):
    ...
    if current_level == 1:
        total_questions = len(easy_a)
    elif current_level == 2:
        total_questions = len(medium_a)
    elif current_level == 3:
        total_questions = len(difficult_a)
        
    st = "Your score is "+str(mark)+"/"+str(total_questions)
    ...
    bpv = Button(text="Previous Level", command=previousLevel, bg='black', fg = "white")
    bpv.pack()

    bnl = Button(text="Next Level", command=nextLevel, bg='black', fg = 'white')
    bnl.pack()
    ...
    sizes = [int(mark),total_questions-int(mark)]
    explode = (0.1,0)
...
def get_all_questions():
    global easy_q, easy_a, medium_q, medium_a, difficult_q, difficult_a
    easy_q, easy_a = get_questions(1)
    medium_q, medium_a = get_questions(2)
    difficult_q, difficult_a = get_questions(3)

    # print(easy_q, easy_a, medium_q, medium_a, difficult_q, difficult_a)

def get_questions(lvl):
    conn = sqlite3.connect('quiz.db')
    create = conn.cursor()
    create.execute('select * from questions where lvl=(?)', str(lvl))
    questions = create.fetchall()
    re_questions = []
    re_ans = []
    for q, a, b, c, d, ans, lvl in questions:
        x = [q, a, b, c, d]
        re_questions.append(x)
        re_ans.append(ans)
    return re_questions, re_ans
